...
3. **AuthzPort**: `npm install @kohaku-ui/authz-hmac` and use `createHmacAuthzPort(secret)` (the sample's HMAC capability token, about 50 lines if you want to read it: `packages/authz-hmac/src/hmac-authz-port.ts`)
4. **StoragePort**: `npm install @kohaku-ui/storage-memory` and start with `createMemoryStoragePort()`; `createFileStoragePort(dataDir)` adds file persistence for lineage / promotions / fixations (what the sample runs). Both are reference implementations; swap in your own store (or a production adapter) later.
