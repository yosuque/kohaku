...
This is the procedure for cutting and publishing a release of the twenty `@kohaku-ui/*` npm packages
...
- [ ] **npmjs.com, for each of the 20 packages below**: Settings → Publishing access → Trusted
  ...
  Packages: `@kohaku-ui/cli`, `client`, `composer`, `data-binding`, `evals`, `host-a2ui`, `host-core`,
  `host-mcp-apps`, `host-rest`, `intents`, `lineage`, `llm`, `otel`, `registry`, `renderer-core`,
  `renderer-react`, `renderer-wc`, `sandbox`, `spec`, `spec-core`. The `release.yml` `npm` job's dry run
  probes every one of these and lists any that are missing a trusted publisher — see §4.
...
- All twenty package manifests bump to the same version (the fixed group).
- All twenty `CHANGELOG.md` files gain a new `## <version>` section.
...
- [ ] Dry run is green: `gh workflow run release.yml -f dry_run=true` run from `main` (no `tag` input on
  ...
  trusted-publisher probe, which must report every one of the 20 packages as `ok`.
...
twenty packages via OIDC trusted publishing with provenance, then `pypi` publishes the wheel/sdist. The
...
- **npm**: `npm deprecate @kohaku-ui/<pkg>@<version> "<reason>"` across all 20 packages is the normal
  ...
  twenty interdependent packages, treat it as effectively unavailable once other packages have started
