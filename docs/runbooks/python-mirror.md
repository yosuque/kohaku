...
design kit: `packages/composer/src/design-system.ts` ⇄ `python/kohaku/src/kohaku/composer/design_system.py`
(vocabulary + fragment), `packages/composer/src/tiers/l2-generate.ts`'s `collectUnknownKitClasses` ⇄
`python/kohaku/src/kohaku/composer/l2_lint.py`'s `collect_unknown_kit_classes`. Their goldens are no longer
hand-written: both `packages/composer/test/design-kit.test.ts` and
`python/kohaku/tests/composer/test_design_kit.py` read the expected "Design kit" prompt fragment from
`spec/test/fixtures/cross-language-canonical.json` (regenerated by
`pnpm --filter @kohaku-ui/spec run generate-cross-language-fixtures` — see "Cross-language golden fixtures"
below), so a change to the fragment is fixed in one place and reflected in both languages' tests rather than
