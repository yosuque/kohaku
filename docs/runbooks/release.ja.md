...
これは、常に足並みを揃えて動く 22 個の `@kohaku-ui/*` npm パッケージと `kohaku-ui` PyPI 配布物
...
- [ ] **GitHub → Settings → Environments**: 既存の `pypi` environment はそのままで良い(任意で同じ `main` +
  ...
  不要です。
- [ ] **npmjs.com、以下の 22 パッケージそれぞれ**: Settings → Publishing access → Trusted publisher →
  ...
  対象パッケージ: `@kohaku-ui/authz-hmac`, `cli`, `client`, `composer`, `data-binding`, `evals`,
  ...
  `release.yml` の `npm` ジョブのドライランはこの 22 件全てをプローブし、未登録のものを列挙します(§4 参照)。
...
- 22 パッケージのマニフェスト全てが同じバージョンに上がっている(fixed グループ)。
- 22 件の `CHANGELOG.md` それぞれに新しい `## <version>` 節が追加されている。
...
- [ ] ドライランが緑であること: `main` から `gh workflow run release.yml -f dry_run=true` を実行する
  ...
  Trusted Publisher プローブも実行し、22 パッケージ全てが `ok` と報告されなければなりません。
...
OIDC trusted publishing + provenance で 22 パッケージを公開し、`pypi` ジョブが wheel/sdist を公開します。
...
- **npm**: 22 パッケージ全てに対する `npm deprecate @kohaku-ui/<pkg>@<version> "<reason>"` が通常の対処法
  です——インストールしようとする人に警告を出しつつ、既にそのバージョンに固定している人を壊しません。
  `npm unpublish` は公開から 72 時間以内で、かつ他の何もそのバージョンに依存していない場合のみ可能です。
  22 パッケージが相互依存する以上、新バージョンへの依存が広がった後は実質使えないと考えてください。
