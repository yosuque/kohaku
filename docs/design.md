...
| admin-react | Governance console (4 review surfaces) as React components. Data only through `client`; theme via renderer-core tokens; promotion preview = direct sandbox mount (sha256-identical). Product knowledge (headers, draft defaults, extra tabs, dictionary) is injected as props | `src/KohakuAdmin.tsx` `src/tabs/` |
...
An append-only `LineageEventRecord {id(ulid), ts, actor{kind: user|model|system}, type, payload}`. View-family (composed / rendered / interacted / fallback), Component-family (generated / used / nominated / schemaSuggested / judged / reviewed / schemaEdited / schemaProposed / published / withdrawn — rejection has no standalone event; it is recorded as `component.reviewed` with `decision: "reject"`), and Fixation-family (`intent.observed` / `intent.fixated` / `intent.unfixated`; `intent.observed` is reserved for a future Intent-frequency feature and is not fired in v0.1).
...
#### Schema suggestion (LLM auto-extraction of the promotion schema, advisory)

At auto-nomination (`evaluateAndList`, i.e. `POST /promotions/evaluate`), `createPromotions`' optional `suggestSchema` hook — the same shape as `judge` — asks an extractor for the registration a reviewer would otherwise type by hand: componentType / intentName / description / props JSON Schema / queryTemplate / emitted events. The reference extractor is `@kohaku-ui/evals`' `createSchemaExtractor` (id `l2-schema-extraction`, version `0.1`, stamped into the result like a rubric), which feeds the model the candidate's HTML behind the same prompt-injection guard as the judge, plus deterministic evidence: the `query://` references literally present in the HTML, the product's supported query paths, the `window.kohaku` allowlist, and the bridge-contract lint issues (`collectL2Issues`). The result is **advisory**: it is persisted on the candidate snapshot (`data.suggestion`, a loose-object field so no spec-core schema changes), audited as `component.schemaSuggested` (model actor, right after `component.nominated`), surfaced on the candidate JSON as an additive optional `suggestion`, and prefilled into the approval form — `approve` still takes the reviewer's own draft, so LIN-PRM-001 and the transition table are untouched. Extraction is **fail-open**: a throw is reported through the promotions `onError` hook (`promotion.suggest.schema`) and the candidate surfaces with an empty form, exactly as before this feature. The extractor lives in evals, not lineage, because lineage must not depend on the LLM layer; lineage owns the `SchemaSuggestion` type and evals returns a structurally identical object (the same idiom as host-rest's `ComponentDraftSchema` mirror of `ComponentDraft`).

**Latency cost**: `POST /promotions/evaluate` runs inside host-rest's per-tenant promotion governance mutex (the same lock approve/reject/withdraw/actions serialize on), and every freshly nominated candidate's extraction now runs **concurrently** rather than sequentially before the batch persists — but the call still does not return, and the lock is not released, until the slowest of those concurrent extractions settles. The fail-open contract above covers a throw; it does not cover a slow or hanging extractor, which delays every other promotion transition for that tenant for as long as this hook takes to resolve.

On the approve path, right after `schema.propose`, lineage diffs the suggested draft against the submitted one field by field (`diffDraft`, canonical JSON so key order is not an edit) and records `component.schemaEdited` (user actor; `changed: []` means the proposal was accepted as-is). The judge sees the suggestion as untrusted evidence for rubric 0.4's `suggestion_fidelity` criterion, and `summarizeLineage` derives two operator numbers from the log: `review` (the `component.nominated → component.reviewed(approve|reject)` turnaround per `(tenant, artifactId)`, nearest-rank quantiles; a `requestChanges` decision does not close a review) and `review.acceptedAsIs` (zero-edit approvals), alongside `promotions.schemaSuggested / schemaEdited`. The admin UI (`@kohaku-ui/admin-react`) shows the proposal's model + confidence, a per-field diff against the reviewer's current form, and a mandatory acknowledgement checkbox that gates the approve button while a suggestion is present — the guard against rubber-stamping a machine proposal. Recorded edits are the raw material for a later few-shot loop (not implemented).
...
- **L2 promotion review** (`judge()` / `l2PromotionRubric`): safety / determinism / a11y / schema_inferability / generality / visual_quality / suggestion_fidelity. Input is HTML + usage record + **runtime telemetry** (via telemetry, `component.used`'s `renderedCount` / `errorCount`; a different axis from the promotion-aggregate uses, transcribing the "reliability of actual rendering" observation into the prompt. The verdict itself still follows `judgeBlocking` as before).
...
`l2PromotionRubric`'s own version history: **0.1 → 0.2** added the `visual_quality` criterion; **0.2 → 0.3** (Task 8) rebalanced weights (`visual_quality` 0.10 → 0.20, `generality` 0.15 → 0.05) and introduced a `safety` floor (veto — see `judge.ts`'s `floor` doc comment for the rationale); **0.3 → 0.4** (B2 Task 5, alongside schema suggestion above) added the `suggestion_fidelity` criterion (weight 0.1) and reweighted `schema_inferability` from 0.15 down to 0.05 to make room for it. **This last shift loosens the default gate for any consumer who upgrades without wiring a `suggestSchema` extractor**: `suggestion_fidelity` scores 1 when no proposal is supplied, so a no-suggestion candidate's score rises by up to +0.10 versus rubric 0.3 (the weight `schema_inferability` lost). Pin `l2PromotionRubricV0_3` to keep the pre-B2 gate. Each superseded version stays pinned as its own exported constant (`l2PromotionRubricV0_1`, `l2PromotionRubricV0_2`, both mirrored in Python; `l2PromotionRubricV0_3`, TypeScript-only — B2 did not touch the Python mirror, so its own pinned rubric constants still stop at `l2_promotion_rubric_v0_2`) so a caller not ready for the resulting score shift can pass it to `judge()` explicitly. Because a verdict recorded under `rubricVersion: "0.1"` was never scored against `visual_quality` at all (a verdict recorded under `"0.1"`/`"0.2"` was also never scored against the current weight rebalance or the `safety` floor, and one recorded under `"0.1"`/`"0.2"`/`"0.3"` was never scored against `suggestion_fidelity`), reconciling an old verdict against the current rubric after the fact is not supported — an operator who wants the whole catalog re-evaluated under the current rubric must **withdraw and re-approve** the affected candidates.
...
- Few-shot reuse of the reviewer edits recorded as `component.schemaEdited` (the extractor prompt is static today)
