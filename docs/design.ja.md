...
| パッケージ | 責務 | 中核ファイル |
|---|---|---|
| spec-core | UI Spec スキーマ・構造検証・Intent 正規化/ハッシュ・diff/patch・cacheKey・**Port 型**・共有の lineage フィルタ述語/limit ヘルパと `normalizeTenant` | `src/schema/spec.ts` `src/ports.ts` `src/diff.ts` `src/lineage-filter.ts` |
...
自動 nomination 時(`evaluateAndList`、すなわち `POST /promotions/evaluate`)、`createPromotions` の任意フック `suggestSchema`(`judge` と同じ形)が抽出器に、レビュアーが手で入力する代わりの登録内容 — componentType / intentName / description / props の JSON Schema / queryTemplate / 発行イベント — を尋ねる。リファレンス実装は `@kohaku-ui/evals` の `createSchemaExtractor`(id `l2-schema-extraction`、version `0.1`。rubric と同様に結果へ刻印される)で、judge と同じプロンプトインジェクション対策のもとで候補の HTML をモデルに渡すほか、決定的な裏付け情報 — HTML に実在する `query://` 参照、プロダクトが対応する query パス、`window.kohaku` の allowlist、bridge-contract lint の指摘(`collectL2Issues`)— も併せて渡す。結果は**あくまで助言**であり、候補スナップショットに永続化され(`data.suggestion`。素の object フィールドなので spec-core のスキーマ変更は不要)、`component.schemaSuggested` として監査され(model actor、`component.nominated` の直後)、候補 JSON には追加の任意項目 `suggestion` として現れ、承認フォームにプリフィルされる — `approve` は依然としてレビュアー自身の draft を受け取るので、LIN-PRM-001 も遷移表も変わらない。抽出は **fail-open** で、throw は promotions の `onError` フック(`promotion.suggest.schema`)経由で報告され、候補はこの機能導入前と同じ空フォームのまま現れる。抽出器を lineage ではなく evals に置くのは、lineage が LLM 層へ依存してはならないため — `SchemaSuggestion` 型自体は `spec-core` が所有する共有の wire-contract 型であり(evals が生成し・lineage が永続化し・client が写す、同一レイヤーのパッケージ同士は依存し合えないため)、他の3パッケージはそれぞれ構造的に同一のオブジェクトを写すか消費する(host-rest の `ComponentDraftSchema` が `ComponentDraft` を写す idiom と同じ)。

**レイテンシコスト**: `POST /promotions/evaluate` は host-rest のテナント別 promotion ガバナンス mutex(approve/reject/withdraw/actions が直列化に使うものと同じロック)の内側で実行され、この scan で新規に nominate された各候補の抽出は `suggestConcurrency`(`createPromotions` 自身のオプション、既定 4。`mapWithConcurrency` によるワーカープール — 完全な逐次でも、無制限の並行でもない)件まで同時に走る。それでも呼び出しは、その中で最も遅い抽出が解決するまで戻らず、ロックも解放されない。上記の fail-open 契約は throw をカバーするものであり、遅い・ハングする抽出器はカバーしない — このフックが解決するまでの間、当該テナントの他の昇格遷移はすべて遅延する。`suggestConcurrency` が抑えるのは、新規 nominate のバーストが LLM プロバイダのレート制限をどれだけ消費するかであり、1 件あたりの呼び出しがどれだけ長く走れるかは抑えない — それは `createSchemaExtractor` 自身の `timeoutMs`(既定 20 秒。`LlmPort.generateObject` に `abort: AbortSignal.timeout(timeoutMs)` を渡す)の役割である。

approve 経路では `schema.propose` の直後に、lineage が提案された draft と提出された draft をフィールド単位で diff し(`diffDraft`。canonical JSON で比較するのでキー順序は差分にならない)、`component.schemaEdited`(user actor。`changed: []` は提案がそのまま承認されたことを意味する)として記録する。`approve` 自身の draft 引数も追加の context(`PromotionJudgeContext.draft`)として配線済みの judge に渡されるため、rubric 0.4 の `suggestion_fidelity`(「schema fidelity」)基準は候補の機械提案だけでなく、実際に登録される schema そのものを検証できる(提案が併存する場合は参考情報としてのみ judge に示される)。`summarizeLineage` はログから 2 つの運用指標を導出する: `review`(`component.nominated → component.reviewed(approve|reject)` の所要時間を `(tenant, artifactId)` ごとに対応付け、nearest-rank の分位で集計。`requestChanges` はレビューを完結させない。この対応付けは入力を `ts` で安定ソートしてから走査するため、呼び出し側が `ts` 昇順を保証する必要はもう無い)と `review.acceptedAsIs`(無編集承認**かつ**確認済みの件数。後述)で、`promotions.schemaSuggested / schemaEdited` も併せて数える。管理 UI(`@kohaku-ui/admin-react`)は提案のモデル名と確信度、レビュアーの現在のフォームとの差分、そして提案が付いている間は承認ボタンをブロックする必須の確認チェックボックスを表示する — 機械の提案をそのまま追認させない歯止めである。このチェックボックスの状態は今はワイヤ上にも送られ(`POST /promotions/:id/approve` の `acknowledgedSuggestion`)、`component.schemaEdited` に `acknowledged` として記録される。**記録するだけで強制はしない**(未指定は `acknowledged: false` として記録され、approve/publish はどちらの値でも進む)。`review.acceptedAsIs` は `changed: []` かつ `acknowledged: true` の両方を満たす記録だけを数える。記録された編集は、後日の few-shot 学習ループ(未実装)の材料になる。
...
`l2PromotionRubric` 自身の改版履歴: **0.1 → 0.2** で `visual_quality` 基準を追加。**0.2 → 0.3**(Task 8)で重みを再配分(`visual_quality` 0.10→0.20 / `generality` 0.15→0.05)し、`safety` の下限(veto)を導入した(根拠は `judge.ts` の `floor` doc コメントを参照)。**0.3 → 0.4**(B2 Task 5、上記のスキーマ提案と同時)で `suggestion_fidelity` 基準(重み 0.1)を追加し、その分の重みを確保するため `schema_inferability` を 0.15 から 0.05 へ再配分した。**この重み変更は、schema を一切渡さない利用者にとってはもう既定ゲートを緩めない。** `judge()` は呼び出しごとに 2 つの rubric variant(`JudgeVerdict.rubricVariant`、`"full"` | `"no-schema"`)から選ぶようになった: 入力に `draft` も `suggestion` も無いときは `suggestion_fidelity` を 1 点で自動採点する代わりに基準そのものを外し、残り 6 基準を `l2PromotionRubricV0_3` と全く同じ重みで採点する(0.4 から `suggestion_fidelity` を外して `schema_inferability` が失った重みを戻すと、他のどの基準の重みも 0.3 と 0.4 の間で変わっていないため、ちょうど 0.3 の重みに一致する)。このとき刻まれる `rubricVersion` は常に `"0.4"` のまま — すなわち**設定されている rubric 自身のバージョン**であり、`"0.3"` に書き換わることはない。`suggestion_fidelity` が外されたことは `rubricVariant: "no-schema"` の側が記録するのであって、バージョン欄の役目ではないためである。したがって `component.judged` の判定を `rubricVersion` で集計する側は、現行 rubric を設定している限り `"0.3"` が現れることはなく、両ケースの区別は `rubricVariant` だけが担う。そのため、この段落がかつて説明していた +0.10 のインフレを避けるために `l2PromotionRubricV0_3` を明示的にピン留めする必要はもう無い(それでも、その rubric — と、その `"0.3"` という刻印そのもの — を無条件に使いたい呼び出し側のために引き続き利用できる)。`approve()` の呼び出し経路に限って言えば、draft は実質的に常に判明している(`approve` 自身の必須引数であり、`PromotionJudgeContext.draft` として渡される)ため、実際のほぼ全ての昇格が目にするのは — 機械提案だけでなく実際に登録される schema に対して `suggestion_fidelity` を採点するようになった — `"full"` variant である。`"no-schema"` が主に対象とするのは、その経路の外で judge が呼ばれる場合(`context.draft` を転送しない独自の `PromotionJudge`、または `evals` の `judge()` をどちらのフィールドも無しで直接呼ぶ場合)である。旧版はそれぞれ `l2PromotionRubricV0_1` / `l2PromotionRubricV0_2`(両言語でエクスポート・ピン留め)、`l2PromotionRubricV0_3`(TypeScript のみ — B2 は Python ミラーに手を入れていないため、Python 側のピン留め済み rubric 定数は `l2_promotion_rubric_v0_2` までしか無い)として残り、スコアの変動に備えられない呼び出し側は `judge()` に明示的に渡せる。`rubricVersion: "0.1"` で記録された判定は `visual_quality` で全く採点されておらず(`"0.1"`/`"0.2"` で記録された判定も現行の重み再配分・`safety` 下限では採点されておらず、`"0.1"`/`"0.2"`/`"0.3"` で記録された判定も `suggestion_fidelity` では採点されていない)、過去の判定を事後的に現行ルーブリックへ突合する仕組みは無いため、既存カタログ全体を現行ルーブリックで評価し直したい運用者は該当候補を**withdraw のうえ再承認**する必要がある。
...
| 44 | 失効ストアの障害は fail-open ではなく fail-closed | `HmacAuthzPort.verify` はストアの拒否を(`AuthzPort.verify` の契約どおり)投げられたエラーとして伝播させ、host-core の `verifyCapabilitySafely` がその throw を `{ kind: "unavailable" }` に変換する。REST はこれを 503 `INTERNAL` に、MCP は構造化ツールエラーに写像する — 失効ストアの障害はアクセスを暗黙に許可するのではなく拒否する |
| 45 | JWKS モードの JWT リゾルバは `audience` を必須にする | `createJwtAuthzPort` の `identity` は `key` が `jwks`/`jwksUrl` かつ `audience` 未設定だと構築時に throw する。JWKS 設定のリゾルバは通常サードパーティの発行者と通信し、他の audience 向けトークンも発行しうるため。加えて HS256 の `secret` は 32 バイト以上、`jwksUrl` は `localhost`/`127.0.0.1`/`::1` を除き `https:` が必須 |
| 46 | L2 昇格ルーブリックにはコール単位の「no-schema」バリアントがある | `@kohaku-ui/evals` の `judge()` は、入力に `draft` も `suggestion` も無いとき、存在しないスキーマに対して採点する代わりに `suggestion_fidelity` 基準を除外して再正規化する。`JudgeVerdict.rubricVariant`(`"full"` \| `"no-schema"`)がコールごとにどちらが起きたかを記録し、`rubricVersion` はどちらのバリアントで採点されたかに関わらず常に設定されたルーブリックのバージョンを表す |
| 47 | スキーマ提案へのレビュアーの確認は記録されるが、強制はされない | `Promotions.approve` の `acknowledgedSuggestion` オプションは `component.schemaEdited` の `acknowledged` フィールドに通され(`summarizeLineage` の `review.acceptedAsIs` が利用する)、サーバ側でそれを設定する強制は何も無い — `approve` へのゲートではなく、意図的な追加の監査シグナルである |
| 48 | サンプルホストはプロセスごとに 1 本のバックエンド接続を StoragePort と失効ストアで共有する | `createPortsFromEnv`(`apps/sample-api/src/ports/from-env.ts`)は redis クライアント / pg `Pool` を厳密に 1 つだけ開き、`client`/`pool` オプション経由で両方に注入する。`createStorageFromEnv` + `createAuthzFromEnv` を別々に呼ぶと接続が 2 本独立に開いてしまう |
| 49 | `kohaku init` は固定のデフォルトではなく生成し立ての capability secret を書き込む | 生成プロジェクトの `.env` には `randomBytes(32).toString("base64url")` が `KOHAKU_CAPABILITY_SECRET` として書き込まれる(`cli/src/init/index.ts`)。以前の共通フォールバック `"dev-secret-change-me"` は廃止し、生成された全プロジェクトが同じ推測可能な secret を持つリスクを閉じた |
| 50 | Postgres の JSON ペイロード列は `jsonb` ではなく `text` にする | `jsonb` はオブジェクトのキーを自身の順序で再直列化してしまい、同一表示保証が前提とするバイト単位の往復一致を壊す。`text` は canonical-JSON のバイト同一性を保存し、これらの列は一切クエリに使われないためコストもゼロである(§11 参照) |
