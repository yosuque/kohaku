...
| `KOHAKU_CAPABILITY_SECRET` | `dev-secret-change-me` | サンプルの HMAC capability 署名鍵。`KOHAKU_STORAGE` が `redis` または `postgres`、あるいは `KOHAKU_AUTHZ` が `jwt` になった時点で、この固定既定値は名前付きの env エラーとして拒否される — 共有バックエンドや検証済み identity 方式が、誰もが知っている秘密で署名した capability を発行し続けるのはクイックスタートの利便性ではなく偽造リスクだからである。既定値が使えるのは単一プロセスの `file`/`memory` + `hmac` というデモ構成のときだけ(`apps/sample-api/src/ports/from-env.ts` の `buildAuthzFromEnv`) |
...
| `KOHAKU_DEMO_ADMIN_ROUTES` | 未設定 | `1` で `KOHAKU_AUTHZ=jwt` のもとでも `POST /api/kohaku/admin/bump-data-version`(§5.5)を登録する(デフォルトでは無効)。有効な bearer トークン + admin ロールは引き続き必須で、これはルートの存在自体だけを切り替える。デフォルトの `hmac` 方式では常にルートが登録される(`apps/sample-api/src/index.ts` の `demoAdminRoutes: … \|\| ports.identity == null`) |
| `KOHAKU_JWT_SECRET` / `KOHAKU_JWT_JWKS_URL` | — | `KOHAKU_AUTHZ=jwt` ではどちらか一方が必須(HS256 共有鍵、または OIDC JWKS エンドポイント)。`KOHAKU_JWT_SECRET` は 32 バイト(UTF-8)以上が必須 — それより短い HS256 共有鍵はコンストラクタ時点で失敗する(`packages/authz-jwt` の `MIN_HMAC_SECRET_BYTES`) |
| `KOHAKU_JWT_ISSUER` / `KOHAKU_JWT_AUDIENCE` | — | `iss` 検証は任意。`KOHAKU_JWT_JWKS_URL` を使う場合は `KOHAKU_JWT_AUDIENCE` が必須(JWKS 経由の発行者は他のオーディエンス向けトークンも発行しうるため)。`from-env.ts` 自身がこの条件をチェックし、ポートを構築する前に `KOHAKU_AUTHZ=jwt with KOHAKU_JWT_JWKS_URL requires KOHAKU_JWT_AUDIENCE` という名前付きエラーを投げる — `@kohaku-ui/authz-jwt` 自身のコンストラクタ時点のチェック(より一般的なメッセージ)は、ポートを直接組み立てる呼び出し側向けのフォールバックとして残る。**推奨: サードパーティの `KOHAKU_JWT_JWKS_URL` を使うときは `KOHAKU_JWT_AUDIENCE` と併せて `KOHAKU_JWT_ISSUER` も設定する** — JWKS URL 自体が発行者ごとに分かれているためリスクは低いが、`issuer` を設定すれば `iss` 検証が URL だけに委ねた暗黙のものではなく、明示的で検証済みの表明になる |
| `KOHAKU_JWT_REQUIRE_TENANT` | `1`(必須) | `KOHAKU_AUTHZ=jwt` のときのみ有効。`1`(既定): `tenant` クレームを持たないトークンを拒否する(`JwtIdentityError` のコードは `MISSING_TENANT`)— 昇格/固定化/Lineage のガバナンス平面はテナントごとに分離されているため、「テナントなし」に黙って倒れるのは、意図的な単一テナント運用よりも発行者の設定ミスである可能性の方がずっと高い。`0` でオプトアウトする(`requireTenant: false` に対応)。このチェックは identity 解決時点、つまりどちらのホストプロファイルがリクエストを見るよりも前に走る: sample-mcp は設計上、解決済みのテナントをその後で捨てる(MCP プロファイルはテナントを解決せず、`intentHash` のみをキーにする)が、それでも `jwt` の下では `0` で明示的にオプトアウトしない限りこのクレームの存在を要求する |
...
| `KOHAKU_PROMOTION_SCHEMA_SUGGEST` | `1`(on) | **sample-api(TS)のみ。** 昇格スキーマの LLM 自動抽出(`createSchemaExtractor`。Admin › Promotions のアドバイザリなプレフィル — [user-guide.md](user-guide.ja.md) §4「When the host wires a schema extractor」参照)のキルスイッチ。`0` にすると extractor の配線自体を無効化する(候補は `AppDeps.schemaExtractor` を省略した場合と同様、提案なしになる)。compose 自体は削らずにこの LLM コストだけ削りたいデプロイのためのもの |
| `KOHAKU_PROMOTION_SCHEMA_SUGGEST_TIMEOUT_MS` | 20000 | **sample-api(TS)のみ。** `createSchemaExtractor` の `timeoutMs`(`@kohaku-ui/evals` 自身の既定値と同じ)に渡す 1 回あたりの抽出予算(ms)。数値化できない値・`0` 以下は既定値にフォールバックする |
