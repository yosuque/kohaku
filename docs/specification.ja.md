...
| `KOHAKU_JWT_REQUIRE_TENANT` | `1`(必須) | `KOHAKU_AUTHZ=jwt` のときのみ有効。`1`(既定): `tenant` クレームを持たないトークンを拒否する(`JwtIdentityError` のコードは `MISSING_TENANT`)— 昇格/固定化/Lineage のガバナンス平面はテナントごとに分離されているため、「テナントなし」に黙って倒れるのは、意図的な単一テナント運用よりも発行者の設定ミスである可能性の方がずっと高い。`0` でオプトアウトする(`requireTenant: false` に対応) |
