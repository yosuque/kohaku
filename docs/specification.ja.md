...
- payload テンプレート: `"$row.<key>"`(クリック行の値)/ `"$value"`(部品の現在値)。描画側がランタイム解決してからホストへ送る。
...
- **L2 へのデザインシステム適用**: `ComposePolicy.designSystem`(`DesignSystemGuide = { tokens?, guidelines?, enforceTokenColors? }`)を配線すると、L2 生成プロンプトに「デザインシステム」節が挿入され、生成物はトークン参照 `var(--kohaku-*)` でスタイルを書く契約になる(値は描画時に sandbox が注入 — SPEC-ENV-003 のテーマ非依存を維持)。`enforceTokenColors`(既定 true)は色直書きを `L2_RAW_COLOR` lint(§8)で修復差し戻しする。**内容の変更・on/off はプロンプト内容の変化なので `generatorVersion` を必ず bump する**(few-shot と同じ運用)。未指定なら挙動・出力バイトとも完全不変。詳細は design.md §8「L2 へのデザインシステム適用」。
