...
| `component.schemaEdited` | artifactId, reviewer, extractorId, extractorVersion, changed(`DraftFieldChange[]`), unchanged(`DraftDiffField[]`) | `approve` 経路で `component.schemaProposed` の直後に **user actor** で記録される。承認された候補が `component.schemaSuggested` を持っていた場合のみ。`changed` はレビュアーが提出した draft が提案と食い違うフィールドの一覧(空 = そのまま承認)、`unchanged` は残りのフィールド |
