...
3. **AuthzPort**: `npm install @kohaku-ui/authz-hmac` を入れて `createHmacAuthzPort(secret)` を使う(サンプルの HMAC capability token、読みたければ約 50 行: `packages/authz-hmac/src/hmac-authz-port.ts`)
4. **StoragePort**: `npm install @kohaku-ui/storage-memory` を入れて `createMemoryStoragePort()` から始める。`createFileStoragePort(dataDir)` は lineage / promotions / fixations のファイル永続化を追加する(サンプルが実際に使っているのはこちら)。どちらも参考実装なので、後から自前のストア(または本番用アダプタ)に差し替えてよい。
