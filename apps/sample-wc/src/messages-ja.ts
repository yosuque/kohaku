import type { RendererMessages } from "@kohaku-ui/renderer-wc";

/**
 * Japanese UI strings for the demo. The library default (DEFAULT_MESSAGES) is English;
 * the sample injects Japanese via <kohaku-surface>'s context.messages.
 * (Same content as sample-web's i18n/messages.ja.ts. Each app keeps its own copy because apps do not share code.)
 */
