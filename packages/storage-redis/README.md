...
- **TLS / least privilege**: use a `rediss://` URL for TLS in transit, and an ACL user scoped to only the commands and key patterns this package actually needs (`GET`/`SET`/`DEL`/`EXISTS`/`INCR`/`INCRBY`/`HSETNX`/`HMGET`/`MGET`/`ZADD`/`ZREM`/`ZRANGE`/`ZREVRANGE`/`MULTI`/`EXEC`/`INFO`/`QUIT` against `{prefix}:*`) rather than a full-access default user. `INFO` and `QUIT` are ioredis's own doing, not this package's: `INFO` is issued once per connection as ioredis's default ready check (`enableReadyCheck`, left at its default), and `QUIT` is what an owned client's `close()` sends first (falling back to a client-side `disconnect()` if that rejects) — deny either and the client never reaches `ready` or never closes gracefully. `PIPELINE` is not a real Redis command (`redis.pipeline()` is client-side batching of the commands already listed here, so it needs no separate ACL entry), and plain `HSET` is not used (only `HSETNX`, for `appendLineage`'s idempotent event write).
