...
export function registerFixationRoutes(app: Hono, ctx: RouteContext): void {
  ...
  /**
   * Shared preamble for the fixation routes (list / proposals / approve / remove): 501 if fixations is not
   * configured, then the governance check for `op`, then an early return. The check order is unchanged: unwired
   * 501 -> governance authorization -> handler. On success, invokes `handler` with the narrowed (non-null)
   * `fixations` API.
   */
  const withFixations = async (
    c: Context,
    op: GovernanceOperation,
    handler: (fixations: FixationsApi) => Promise<Response>,
  ): Promise<Response> => {
    ...
    return handler(fixations);
  };

  app.get("/fixations", (c) =>
    withFixations(c, { kind: "fixation.list" }, async (fixations) =>
      c.json({ fixations: await fixations.list(await tenantScope(c, deps)) }),
    ),
  );

  app.get("/fixations/proposals", (c) =>
    withFixations(c, { kind: "fixation.proposals" }, async (fixations) =>
      c.json({ proposals: await fixations.proposals(await tenantScope(c, deps)) }),
    ),
  );

  app.post("/fixations/approve", (c) =>
    withFixations(c, { kind: "fixation.approve" }, async (fixations) => {
  ...
  app.post("/fixations/:intentHash/remove", (c) =>
    withFixations(
      c,
      { kind: "fixation.remove", intentHash: c.req.param("intentHash") },
      async (fixations) => {
        const principal = await getPrincipal(c);
        const tenant = await resolveTenant(c, deps);
        try {
